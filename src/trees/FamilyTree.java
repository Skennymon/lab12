...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        TreeNode getNodeWithName(String targetName)
        {
            ...
            {
                // If child.getNodeWithName(tar	getName) returns a non-null node,
                // then that's the node we're looking for. Return it.
            	
            	TreeNode val = child.getNodeWithName(targetName);
            	if(val!=null) {
            		return val;
            	}
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) {
			throw new TreeException("There should be a colon in the string but there isn't");
		}
		...
		if (root == null) {
			...
			if(parentNode==null) {
				throw new TreeException("Node not found with the name given");
			}
...
//				data file. Throw an exception.
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Unable to find node with first input");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Unable to find node with second input");
